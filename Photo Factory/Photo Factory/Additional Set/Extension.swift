...
extension CIImage{
    ...
    var aspectRadio: AspectRadio {
        let radio: Double = Double(self.extent.width) / Double(self.extent.height)
...
extension CGRect{
    func expand(size: Int) -> CGRect {
        return CGRect(x: 0, y: 0, width: self.width + CGFloat(size), height: self.height + CGFloat(size))
    }
}
